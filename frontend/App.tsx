...
export const App = () => {
    ...
    const handleGpuVramUpdate = (newVram: Uint32Array) => {
        // We can choose to sync it to the React state for debug inspection,
        // or just let the GPUCanvas handle the rendering.
        // For now, let's sync it so other tools (like the hex dump) see it.
        // But doing this at 60fps might be heavy for React.
        // Let's do it throttled or just let GPUCanvas render its own canvas.
        // Actually, the user wants "Phase 0: Prove the GPU Layer".
        // Let's rely on GpuCanvas to render itself, and maybe sync state occasionally?
        // Wait, the plan said "GpuCanvas... integrates GpuRuntime... renders to <canvas>".
        // So when GPU mode is active, we should hide the div-grid and show GpuCanvas.
        setVram(newVram);
    };
    ...
    return (
        <div style={{ background: '#000', color: '#0f0', fontFamily: 'monospace', padding: '20px' }}>
            ...
            <div style={{
                ...
                {useGpuCanvas ? (
                    <GpuCanvas onVramUpdate={handleGpuVramUpdate} />
                ) : (
            ...
            <div style={{ marginTop: '10px', borderTop: '1px solid #333', paddingTop: '5px' }}>
                ...
                >
                    {showLauncher ? 'Hide Launcher' : 'Show TS Launcher'}
                </button>
            </div>
