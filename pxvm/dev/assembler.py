...
from pxvm.utils.layout import calculate_image_size, allocate_rows, write_quantized_matrix
...
class PixelAssembler:
    ...
    def compile(self) -> np.ndarray:
        ...
        dims = {name: (c, r) for name, (r, c, _) in self.matrices.items()}

        # 2. Calculate minimum image dimensions
        from pxvm.utils.layout import calculate_matrix_rows_quantized
        height = 1 # instruction row
        for name, (cols, rows) in dims.items():
            height += calculate_matrix_rows_quantized(cols, rows, self.image_width)
        ...
        for name, (rows, cols, data) in self.matrices.items():
            if data is not None:
                ...
                print(f"Writing quantized data for {name} ({rows}x{cols}) to row R{row_start}")
                write_quantized_matrix(img, row_start, data, image_width=width)
