...
class PxVM:
    def __init__(self, width=1024, height=1024):
        ...
        self.glyph = np.zeros((height, width), dtype=np.uint16)
    ...
    def step(self):
        ...
        # Render glyphs to framebuffer (simple visualization)
        glyph_coords = np.where(self.glyph > 0)
        self.framebuffer[glyph_coords] = [128, 128, 128] # Grey for glyphs
        ...
        for kernel in self.kernels[:]:
            ...
            if opcode == 0:   # HALT
                ...
                if 0 <= y < self.height and 0 <= x < self.width:
                    ...
                    kernel.regs[0] = 0
            elif opcode == 102: # SYS_WRITE_GLYPH
                x, y, g = kernel.regs[0], kernel.regs[1], kernel.regs[2]
                if 0 <= y < self.height and 0 <= x < self.width:
                    self.glyph[y, x] = g & 0xFFFF
            elif opcode == 103: # SYS_READ_GLYPH
                x, y = kernel.regs[0], kernel.regs[1]
                if 0 <= y < self.height and 0 <= x < self.width:
                    kernel.regs[0] = self.glyph[y, x]
                else:
                    kernel.regs[0] = 0
            elif opcode == 104: # SYS_SPAWN
                code_ptr = kernel.regs[0]
                x, y = kernel.regs[1], kernel.regs[2]

                # For now, child inherits parent's full memory
                child_code = kernel.memory

                # Spawn a new kernel, but don't get its PID yet
                new_kernel = Kernel(self.next_pid, b'', kernel.color)
                new_kernel.memory[:] = child_code
                new_kernel.pc = 0
                new_kernel.regs[0] = x
                new_kernel.regs[1] = y
