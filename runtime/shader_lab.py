...
import numpy as np
import openai
from jit_compiler import jit_compiler, CompilationError
...
def execute_wgsl_on_backend(wgsl_code):
    try:
        # Use JIT Compiler to compile/validate first
        shader = jit_compiler.compile(wgsl_code)

        # If compilation succeeded, proceed to execution
        ...
        print(f"JIT Compilation Error: {e.message}")
...
def generate_shader():
    ...
    try:
        ...
        return jsonify({
            "status": "error",
            "error": f"JIT Compilation Failed: {e.message}",
            "line": e.line_number,
